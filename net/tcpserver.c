    ...
    t_tcpserver_socketreceivefn socketreceivefn)
{
    ...
    if (!x)
    {
        pd_error(x, "%s_socketreceiver: unable to allocate %zu bytes", objName, sizeof(*x));
    }
    else
    {
        ...
        if (!(x->sr_inbuf = malloc(INBUFSIZE)))
        {
            ...
            x = NULL;
            pd_error(x, "%s_socketreceiver: unable to allocate %ld bytes", objName, INBUFSIZE);
        }
    }
    return (x);
}
...
//    int                             sender_thread_result;
    ...
    for (i = j = 0; i < ttbp->argc; ++i)
    {
        ...
        { /* load floats into buffer as long as they are integers on [0..255]*/
            ...
            if (e != 0)
            {
                pd_error(x, "%s_broadcast_thread: item %d (%f) is not an integer", objName, i, f);
                if (NULL != arg) freebytes (arg, sizeof (t_tcpserver_broadcast_params));
                return NULL;
            }
            if ((d < 0) || (d > 255))
            {
                pd_error(x, "%s_broadcast_thread: item %d (%f) is not between 0 and 255", objName, i, f);
                if (NULL != arg) freebytes (arg, sizeof (t_tcpserver_broadcast_params));
                return NULL;
            }
            c = (unsigned char)d; /* make sure it doesn't become negative; this only matters for post() */
...
//LOOP
                ...
                for(client = 0; client < ttbp->x->x_nconnections; client++)	/* check if connection exists */
                {
                    ...
                    {
                        pd_error(x, "%s_broadcast_thread: buffer too small for client(%d)", objName, client);
                        if (NULL != arg) freebytes (arg, sizeof (t_tcpserver_broadcast_params));
                        return NULL;
                    }
#endif // SIOCOUTQ
        ...
        { /* symbols are interpreted to be file names; attempt to load the file and send it */
            ...
            if (fptr == NULL)
            {
                pd_error(x, "%s_broadcast_thread: unable to open \"%s\"", objName, fpath);
                if (NULL != arg) freebytes (arg, sizeof (t_tcpserver_broadcast_params));
                return NULL;
            }
            rewind(fptr);
...
//LOOP
        ...
        { /* arg was neither a float nor a valid file name */
            pd_error(x, "%s_broadcast_thread: item %d is not a float or a file name", objName, i);
            if (NULL != arg) freebytes (arg, sizeof (t_tcpserver_broadcast_params));
            return NULL;
        }
    }
