...
static void *udpsend_tilde_new(t_floatarg inlets, t_floatarg blocksize)
{
    ...
    if (x)
    {
        ...
        {
            pd_error(x, "udpsend~: Number of channels must be between 1 and %d", DEFAULT_AUDIO_CHANNELS);
            return NULL;
        }
        ...
        if (!x->x_myvec)
        {
            pd_error(x, "udpsend~: out of memory");
            return NULL;
        }
        ...
        {
            pd_error(x, "udpsend~: blocksize must fit snugly in %d", DEFAULT_AUDIO_BUFFER_SIZE);
            return NULL;
        } 
