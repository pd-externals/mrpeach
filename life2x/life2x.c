...
static void *life2x_new(t_symbol *s, short ac, t_atom *av)
{
    ...
    { /* if there is at least 1 argument */
        ...
        { /* if first arg is an int */
            ...
            if (x->l_xcellnum < 4)
            {
                pd_error(x, "Life: first argument < 4, set to 4");
                x->l_xcellnum = 4; /* min # of  horiz cells */
            }
            if (x->l_xcellnum > MAXSIZE)
            {
                pd_error(x, "Life: first argument > %d, set to %d", MAXSIZE, MAXSIZE);
            ...
            { /* if 2nd arg is an int */
                ...
                if (x->l_ycellnum < 4)
                {
                    pd_error(x, "Life: 2nd argument < 4, set to 4");
                    x->l_ycellnum = 4; /* min # of verti cells */
                }
                if (x->l_ycellnum > MAXSIZE)
                {
                    pd_error(x, "Life: 2nd argument > %d, set to %d", MAXSIZE, MAXSIZE);
                    x->l_ycellnum = MAXSIZE; /* max # of verti cells */
                }
            }
            else if (ac > 1)
            { /* if 2nd arg not an int */
                pd_error(x, "Life: 2nd argument must be int");
        ...
        { /*if first arg not an int */
            pd_error(x, "Life: first argument must be int");
    ...
    {
        pd_error (x, "Unable to allocate memory for the life array (%luX%lu needs %lu bytes)",
    ...
    {
        pd_error(x, "life2x_new: Unable to allocate %lu bytes for column list",
    ...
    {
        pd_error(x, "life2x_new: Unable to allocate %lu bytes for column outlets",
